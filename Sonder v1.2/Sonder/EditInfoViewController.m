...
- (void)viewDidLoad
{
    ...
    [query getObjectInBackgroundWithId:[[PFUser currentUser] objectId] block:^(PFObject *object, NSError *error) {
        ...
        self.school.text = [object objectForKey:@"school"];
        self.hometown.text = [object objectForKey:@"bio"];
        
        
        if ([self.relationship isEqualToString:@"Private"]) {
            [self.relation setSelectedSegmentIndex:0];
            
        }
        else if ([self.relationship isEqualToString:@"Single"]){
            [self.relation setSelectedSegmentIndex:1];
        }
        else if ([self.relationship isEqualToString:@"Taken"]){
            [self.relation setSelectedSegmentIndex:2];
        }
