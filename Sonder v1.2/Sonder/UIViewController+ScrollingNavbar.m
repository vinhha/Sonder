...
{
	CGRect frame = self.navigationController.navigationBar.frame;
    CGRect tabFrame = self.tabBarController.tabBar.frame;// Tab Bar
    //CGFloat tabDelta = delta;//Tab Bar
	...
	if (delta > 0) {
      ...
      //      tabDelta = tabFrame.origin.y - self.deltaLimit;//Tab Bar
            NSLog(@"toller");
		}
		
		frame.origin.y = MAX(-self.deltaLimit, frame.origin.y - delta);
        tabFrame.origin.y = [[UIScreen mainScreen] bounds].size.height - tabFrame.size.height;//MIN([[UIScreen mainScreen] bounds].size.height + tabFrame.size.height, tabFrame.origin.y + tabDelta);//Tab Bar
		self.navigationController.navigationBar.frame = frame;
       self.tabBarController.tabBar.frame = tabFrame;//Tab Bar
       // NSLog(@"%f That..", [[UIScreen mainScreen] bounds].size.height + tabFrame.size.height);
...
//            }
        ...
        //    tabDelta = delta;//Tab Bar
            NSLog(@"tallor");
		}
        
//        if (tabFrame.origin.y > 529) {
//            tabFrame.origin.y =[[UIScreen mainScreen] bounds].size.height + tabFrame.size.height;
//            NSLog(@"%f twenty", tabFrame.origin.y);
//        }

		frame.origin.y = MIN(20, frame.origin.y - delta);
        tabFrame.origin.y = [[UIScreen mainScreen] bounds].size.height - tabFrame.size.height;//MAX([[UIScreen mainScreen] bounds].size.height - tabFrame.size.height, tabFrame.origin.y + tabDelta); //Tab Bar
      //  NSLog(@"%f This..", tabFrame.origin.y);
		self.navigationController.navigationBar.frame = frame;
...
- (void)checkForPartialScroll
{
	...
	if (pos >= -2) {
		[UIView animateWithDuration:0.2 animations:^{
			CGRect frame;
            CGRect tabFrame;//Tab Bar
            tabFrame = self.tabBarController.tabBar.frame;// Tab Bar
			frame = self.navigationController.navigationBar.frame;
			CGFloat delta = frame.origin.y - self.statusBar;
            //CGFloat tabDelta = tabFrame.size.height;//Tab Bar
			frame.origin.y = MIN(20, frame.origin.y - delta);
            tabFrame.origin.y = [[UIScreen mainScreen] bounds].size.height - tabFrame.size.height;//MIN([[UIScreen mainScreen] bounds].size.height - tabFrame.size.height, tabFrame.origin.y + tabDelta);//Tab Bar
			self.navigationController.navigationBar.frame = frame;
            self.tabBarController.tabBar.frame = tabFrame;
		...
		[UIView animateWithDuration:0.2 animations:^{
           ...
           // NSLog(@"%f", tabDelta);
			frame.origin.y = MAX(-self.deltaLimit, frame.origin.y - delta);
           // tabFrame.origin.y = MAX([[UIScreen mainScreen] bounds].size.height, tabFrame.origin.y + tabDelta);//Tab Bar
            self.tabBarController.tabBar.frame = tabFrame;//Tab Bar
			self.navigationController.navigationBar.frame = frame;
			self.expanded = NO;
			self.collapsed = YES;
...
- (void)updateNavbarAlpha:(CGFloat)delta
{
    ...
    self.tabBarController.tabBar.alpha = alpha;//Tab Bar
