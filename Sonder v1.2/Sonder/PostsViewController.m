...
{
    NSLog(@"numberOfRowsInSection: %lu", (unsigned long)[self.objects count]);
    self.counter2 = [NSNumber numberWithInt:0];
...
- (CGFloat) tableView: (UITableView *) tableView heightForRowAtIndexPath: (NSIndexPath *) indexPath {
    ...
    NSLog(@"Got Height: %f", height);
    
//    NSArray *objects = self.objects;
//    
//    //NSLog(@"%lu", (unsigned long)objects.count);
//    PFObject *indexObject = [objects objectAtIndex:indexPath.row];
//    NSArray *postFlags = [indexObject objectForKey:@"flaggedBy"];
//
//    if (height == 0.0 && [postFlags containsObject:[[PFUser currentUser] objectId]]) {
//        [self refresh];
//    }
    return height;
}
...
//    NSIndexPath *indexPath = [self.tableView indexPathForRowAtPoint:buttonPosition];
    ...
    //    //NSLog(@"%lu", (unsigned long)objects.count);
    PFObject *indexObject = [objects objectAtIndex:indexPath.row];
    self.allCompliments = [NSMutableArray arrayWithArray:[indexObject objectForKey:@"thoughtCompliments"]];
    ...
    NSLog(@"allCompliments: %@", self.allCompliments);
...
-(IBAction)filter:(id)sender{
    ...
    if (self.tag == [NSNumber numberWithInt:0]) {
        ...
        NSArray *itemArray = [NSArray arrayWithObjects: @"New", @"Near", @"Girls", @"Guys",@"Top", nil];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(UIButton *)sender {
    ...
    if ([segue.identifier isEqualToString:@"commentsSegue"]) {
        ...
        CommentViewController *postComments = [segue destinationViewController];

        NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
        PFObject *object = [self.objects objectAtIndex:indexPath.row];
        
        PFQuery *query = [PFQuery queryWithClassName:@"Comments"];
        [query whereKey:@"OP" equalTo:[object objectId]];
        [query includeKey:@"_Comments"];
        
        postComments.commentsObject = [query getFirstObject];

        postComments.thoughtObject = object;
